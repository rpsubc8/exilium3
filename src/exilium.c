...
#define maxArticulos 60       //Soporte para 60 articulos
...
DATAFILE *datosIntro; //Contiene los datos de la INTRO
...
int valorRaton;
int parametroNoSonido = FALSE;
int tipoPantalla = GFX_AUTODETECT;
int parametroIntro= FALSE;


DIALOG dialogo[] =
{
   /* (dialog proc)     (x)   (y)   (w)   (h)   (fg)  (bg)  (key) (flags)  (d1)       (d2)  (dp)               (dp2) (dp3) */
//   { d_clear_proc,      100,   100,    100,    100,    255,  255,    0,    0,       0,         0,    NULL,              NULL, NULL  },
   { d_shadow_box_proc, 5,   10,   100, 70,   0,    237,    0,    0,       0,         0,    NULL,              NULL, NULL  },
   { d_button_proc,     30,  71,  100,  20,   0,    243,    KEY_A,    D_EXIT,       0,         0,    "&Aceptar",       NULL, NULL },
   { d_check_proc,      10,   15,   75,  10,   0,    237,    0,    0,      0,         0,    "VENTANA",       NULL, NULL },
   { d_check_proc,      15,   30,   75,  10,   0,    237,    0,   D_SELECTED,      0,         0,    "PORTADA",       NULL, NULL },
   { d_check_proc,      20,   45,   60,  10,   0,    237,    0,    0,       0,         0,    "SETUP",   NULL, NULL  },
   { d_check_proc,      25,   60,   60,  10,   0,    237,    0,    0,       0,         0,    "INTRO",   NULL, NULL  },
...
void ReproduceSample(SAMPLE *sonido);
void AjustaPaleta(PALETTE paleta,short int bucle);
void EfectoTelevision(BITMAP *pantalla);
...
int main(int argc,char *argv[]){
 ...
 set_window_title("Diskmag Exilium 3");
 ...
 for (i=0;i<argc;i++){
  ...
  if (stricmp(argv[i],"8")==0)
    parametro8 = TRUE;
  if ((stricmp(argv[i],"nosonido")==0)||(stricmp(argv[i],"NOSONIDO")==0))
    parametroNoSonido = TRUE;
 }
 ...
 if (argc==1){
  ...
  set_palette(configuracion[PALETA_OPCIONES].dat);
  blit(configuracion[FOTO_OPCIONES].dat,screen,0,0,0,0,320,200);
  
  ret = do_dialog(dialogo, -1);
  #ifndef ALLEGRO_DOS
   if (dialogo[2].flags == D_SELECTED)
    tipoPantalla = GFX_AUTODETECT_WINDOWED;
   else
    tipoPantalla = GFX_AUTODETECT_FULLSCREEN;
  #endif
  if (dialogo[3].flags == D_SELECTED)
   parametroNo = FALSE;
  else
   parametroNo = TRUE;
  if (dialogo[4].flags == D_SELECTED)
   parametroSetup= TRUE;
  else
   parametroSetup= FALSE;
  if (dialogo[5].flags == D_SELECTED)
   parametroIntro= TRUE;
  else
   parametroIntro= FALSE;
 }


 if (parametroNoSonido == FALSE){
...
//  #ifndef ALLEGRO_WINDOWS
   if (parametroSetup){
    if (set_gfx_mode(tipoPantalla,640,480,0,0)!=0){
...
//  #endif
 ...
 //Aqui el resto de archivos a leer
 packfile_password(cadenaClave);
  LeerArchivoDatos("datos.dat",&datafile);
  LeerArchivoDatos("sliders3.dat",&revista);
 packfile_password(NULL);
 ...
 #ifndef ALLEGRO_QNX
  if (parametroIntro==TRUE) MuestraIntro();
 #endif
 if (parametro8){
  set_color_depth(8);
  grises = TRUE; 
 ...
 if (set_gfx_mode(tipoPantalla,anchoX,altoY,0,0)!=0){
  if (set_gfx_mode(GFX_SAFE,anchoX,altoY,0,0)!=0){
   allegro_message("Error en modo de video\nCambie driver grafico en CFG\nO pruebe en escala de grises\n");
   allegro_exit();
   //printf("Error en modo de video\nCambie driver grafico en CFG\nO pruebe en escala de grises");
   exit(-1);
...
void ControlaEventos(){
 ...
 if (driverSonido!=FALSE){
  play_sample(configuracion[SAMP_SALIR].dat,255,127,1000,FALSE);
  rest(1300);
 } 
}
...
/************************************************/
void SiguienteCancion(){
 if ((driverSonido!=FALSE)&&(sonido!=FALSE)){
  stop_midi();
...
*/
 while ((crono<4)&&!keypressed()); //Espera a fundir a los 13 segundos
 //fade_out(1);
 clear_keybuf();
 Fundir();
...
void Despedida(DATAFILE *datos){
 ...
 blit(foto,screen,0,0,0,0,foto->w,foto->h);
 masked_blit(diskmag,screen,0,0,150,26,diskmag->w,diskmag->h);
 masked_blit(exilium,screen,0,0,438,265,exilium->w,exilium->h);
 masked_blit(issue,screen,0,0,4,432,issue->w,issue->h);
 crono=0;
 LOCK_VARIABLE(crono);
 LOCK_FUNCTION(inc_crono);
 install_int(inc_crono,1000); //Arranco el temporizador de segundos
...
/************************************************/
void MostrarLineaComandos(void){
 allegro_message("\n Diskmag Exilium issue #2   Grupo: SLIDERS   Fecha: 15-10-2003\n==============================================================\n\n exilium [8|16] [no] [/?|?] [setup] [nosonido]\n\n   8       --> 8 bits de color\n  16       --> 16 bits de color\n  no       --> No visualiza la presentacion\n  setup    --> Muestra el menu de configuracion\n  nosonido --> Sin sonido\n  /?    --> Muestra ayuda\n\nEjemplo: exilium 8 no        Sin prensentacion y 256 colores\n         exilium /?          Muestra esta ayuda\n         exilium setup       Carga el menu de configuracion\n\nPor defecto arranca en 16 bits de color y con presentacion\n");
}
...
void AccionBarraDesplegable(){
 ...
 int i;
  ...
  ReproduceSample(configuracion[SAMP_OCULTA].dat);
...
if (raton){
   ...
   if ((mouse_b & 2)&&(*ratonY!=0)){
     if (menuArticulosDesplegado==FALSE){
       if (menuBuscar==FALSE){
        if (*ratonY>0){
          ReproduceSample(configuracion[SAMP_CURSOR].dat);
          ScrollArriba();
        }
        if (*ratonY<0){
          ReproduceSample(configuracion[SAMP_CURSOR].dat);
   ...
   if (mouse_b & 1) {
     if ((mouse_y>=447)&&(mouse_y<=468)&&(mouse_x>=120)&&(mouse_x<=142)&&(driverSonido!=FALSE)){
      ...
      ReproduceSample(configuracion[SAMP_SONIDO].dat);
     ...
     if ((mouse_y>=447)&&(mouse_y<=468)&&(mouse_x>=120)&&(mouse_x<=142)&&(driverSonido!=FALSE)) AccionSonido();
      ...
      if ((mouse_y>=434)&&(mouse_y<=445)&&(mouse_x>=295)&&(mouse_x<=335)&&(menuArticulosDesplegado==FALSE)&&(menuBuscar==FALSE)){
        ReproduceSample(configuracion[SAMP_MENU].dat);
        AccionMenuDesplegable();
      }
      else{
       if ((mouse_y>=474)&&(mouse_y<=479)&&(mouse_x>=295)&&(mouse_x<=335)&&(menuArticulosDesplegado!=FALSE)&&(menuBuscar==FALSE)){
         ReproduceSample(configuracion[SAMP_MENU].dat);
         AccionMenuDesplegable();
       }
       else{
        if ((mouse_y>=460)&&(mouse_y<=467)&&(mouse_x>=447)&&(mouse_x<=454)&&(menuArticulosDesplegado!=FALSE)){
         ...
         if ((mouse_y>=467)&&(mouse_y<=477)&&(mouse_x>=447)&&(mouse_x<=454)&&(menuArticulosDesplegado!=FALSE)){
          ...
          if ((mouse_y>=465)&&(mouse_y<=477)&&(mouse_x>=175)&&(mouse_x<=182)&&(menuArticulosDesplegado!=FALSE)&&(menuBuscar==FALSE)){
            ReproduceSample(configuracion[SAMP_MENU].dat);
            AccionMenuDesplegable();
            ReproduceSample(configuracion[SAMP_CARGA].dat);
            EliminarArticulo();
            InicializarArticulo();
...
void ControlaEventoTeclado(boolean * salir){
 ...
 if (keypressed()){
  ...
  switch(scancode){
   ...
   case KEY_UP: ReproduceSample(configuracion[SAMP_CURSOR].dat);
                if ((menuArticulosDesplegado==FALSE)&&(menuBuscar==FALSE))
                  ScrollAbajo();
                else
                  ScrollArribaArticulos(&losArticulos,letras);
                break;
   case KEY_DOWN: ReproduceSample(configuracion[SAMP_CURSOR].dat);
                  if ((menuArticulosDesplegado==FALSE)&&(menuBuscar==FALSE))
                    ScrollArriba();
                  else
                    ScrollAbajoArticulos(&losArticulos,letras);                  
                  break;
   case KEY_PGUP: ReproduceSample(configuracion[SAMP_SCROLL].dat);
                  if ((menuArticulosDesplegado==FALSE)&&(menuBuscar==FALSE))
                    ScrollAbajo30Lineas();                  
                  break;
   case KEY_PGDN: ReproduceSample(configuracion[SAMP_SCROLL].dat);
                  if ((menuArticulosDesplegado==FALSE)&&(menuBuscar==FALSE))
                    ScrollArriba30Lineas();                  
                  break;
   case KEY_LEFT: ReproduceSample(configuracion[SAMP_CURSOR].dat);
                  if (menuArticulosDesplegado !=FALSE)
                    ScrollArribaArticulos(&losArticulos,letras);                  
                  break;
   case KEY_RIGHT: ReproduceSample(configuracion[SAMP_CURSOR].dat);
                   if (menuArticulosDesplegado !=FALSE)
                     ScrollAbajoArticulos(&losArticulos,letras);                   
                   break;
   case KEY_F12: ReproduceSample(configuracion[SAMP_FOTO].dat);
                 CapturarPantalla();
                 break;
   case KEY_F10: ReproduceSample(configuracion[SAMP_FOTO].dat);
                 CapturarPantalla();
                 break;
   case KEY_PLUS_PAD: ReguladorVolumen(mas); break;
   case KEY_MINUS_PAD: ReguladorVolumen(menos); break;
   case KEY_S: ReproduceSample(configuracion[SAMP_SONIDO].dat);
               AccionSonido();
               break;
   case KEY_M: ReproduceSample(configuracion[SAMP_MENU].dat);
               if (menuBuscar==FALSE)
                 AccionMenuDesplegable();
               break;
   case KEY_F3: ReproduceSample(configuracion[SAMP_SONIDO].dat);
                AccionBuscar(salir);
                break;
   case KEY_END: if ((menuBuscar==FALSE)&&(menuArticulosDesplegado==FALSE)){
                  ...
                  ReproduceSample(configuracion[SAMP_INIFIN].dat);
                  ScrollArriba30Lineas();
                 }
                 break;
   case KEY_HOME: if ((menuBuscar==FALSE)&&(menuArticulosDesplegado==FALSE)){                   
                  ...
                  break;
   case KEY_A: ReproduceSample(configuracion[SAMP_SONIDO].dat);
               numMidiActual--;
               SiguienteCancion(); break;
   case KEY_D: ReproduceSample(configuracion[SAMP_SONIDO].dat);
               numMidiActual++;
               SiguienteCancion(); break;
  }
  clear_keybuf();
...
void ControlaEventoJoystick(){
 ...
 if (palancaJuegos!=FALSE){
  ...
  if (joy[0].stick[0].axis[0].d1){
   if ((menuArticulosDesplegado==FALSE)&&(menuBuscar==FALSE)){
    ReproduceSample(configuracion[SAMP_SCROLL].dat);
    ScrollAbajo30Lineas();
   }
...
void ReproduceSample(SAMPLE *sonido){
 if (driverSonido!=FALSE)
  play_sample(sonido,numVolumenActual,128,1000,FALSE);
}
...
void MuestraIntro(){
//Muestra la INTRO
 int azulFondo, azulLetra;
 short int i,a,x,y;
 BITMAP *pantallaVirtual;
 MIDI *cancion;
 BITMAP * auxFoto;
 double auxFlotante;
 PALETTE paletaGris;
 ...
 packfile_password(cadenaClave);
  LeerArchivoDatos("intro.dat",&datosIntro);
 packfile_password(NULL);

 set_color_depth(8);
 if (set_gfx_mode(tipoPantalla,320,200,0,0)!=0){
  if (set_gfx_mode(GFX_SAFE,320,200,0,0)!=0){
   allegro_message("Error en modo de video\nCambie de driver grafico en CFG\n");
   allegro_exit();
   //printf("Error en modo de video\nCambie de driver grafico en CFG");
...
// set_palette(black_palette);
 ...
 azulFondo = makecol(124,112,218);
 azulLetra = makecol(62,49,162);

 pantallaVirtual = create_bitmap(320,200);
 rest(2000);
 if (pantallaVirtual != NULL){
  clear_bitmap(pantallaVirtual);
  crono = 0;
  LOCK_VARIABLE(crono);
  LOCK_FUNCTION(inc_crono);
  install_int(inc_crono,1000); //Arranco el temporizador de segundos

  rectfill(pantallaVirtual,0,0,319,199,azulFondo);
  rectfill(pantallaVirtual,20,20,299,179,azulLetra);
  textout_ex(pantallaVirtual,font,"**** COMOORDORE 64 JAVA V2 ****",38,25,azulFondo,azulLetra);
  textout_ex(pantallaVirtual,font,"64G RAM SYSTEM  2 JAVA BYTES FREE",30,40,azulFondo,azulLetra);
  textout_ex(pantallaVirtual,font,"READY.",22,55,azulFondo,azulLetra);
  blit(pantallaVirtual,screen,0,0,0,0,pantallaVirtual->w,pantallaVirtual->h);
  play_midi(datosIntro[MIDI_INTRO].dat,FALSE);

  while (crono<2);
  play_sample(datosIntro[SONIDO_CHIQUITO_GRITO].dat,255,127,1000,FALSE);
  textout_ex(screen,font,"LOAD\".INTRO.JAVA\",8,1",22,70,azulFondo,azulLetra);
  while (crono<4);
  textout_ex(screen,font,"READY.",22,85,azulFondo,azulLetra);// while (crono<4);
  while (crono<5);
  textout_ex(screen,font,"RUN",22,95,azulFondo,azulLetra);// while (crono<5);
  while (crono<6);
  textout_ex(screen,font,"LOADING DAEMON \"SPRITES\"",22,110,azulFondo,azulLetra);// while (crono<6);
  while (crono<7);
  textout_ex(screen,font,"LOADING DAEMON \"VIRUS\"",22,120,azulFondo,azulLetra);// while (crono<7);
  while (crono<8);
  textout_ex(screen,font,"LOADING DAEMON \"SOUND\"",22,130,azulFondo,azulLetra);// while (crono<8);
  while (crono<9);
  textout_ex(screen,font,"VIRTUAL MACHINE ENABLED",22,140,azulFondo,azulLetra);// while (crono<10);
  while (crono<10);
  play_sample(datosIntro[SONIDO_CHIQUITO_NOPUEDESER].dat,255,127,1000,FALSE);
  textout_ex(screen,font,"ERROR!! YOU DON'T HAVE ANY MEMORY",22,155,makecol(128,0,128),azulLetra);
  while (crono<11);
  textout_ex(screen,font,"YOU NEED 128 GB OF RAM FREE",22,165,makecol(200,0,128),azulLetra);
  while(crono<13);

  //Copiamos todo lo que hay en pantalla al buffer y trabajamos con el
  blit (screen,pantallaVirtual,0,0,0,0,pantallaVirtual->w,pantallaVirtual->h);
  for (i=0;i<130;i++){
   vsync(); blit (pantallaVirtual,screen,20+i,25,20,25,130-i,175);
  }

  for (i=0;i<61;i++){
   vsync(); blit (pantallaVirtual,screen,150,69,150-(i<<1),69+i,40,8);
  }

  for (i=0;i<75;i++){
   vsync(); blit (pantallaVirtual,screen,150,130,150-i,130,50,8);
  }

  for (i=0;i<25;i++){
   vsync(); blit (pantallaVirtual,screen,150,140,150-i,140,58,8);
  }
  
  remove_int(inc_crono);
 }

 auxFoto = datosIntro[FOTO_JEEPERS_CINEMA].dat;
 clear_bitmap (screen);
 set_palette(black_palette);
 
 blit (auxFoto,screen,0,0,70,25,auxFoto->w,auxFoto->h);
 Desfundir (datosIntro[PALETA_JEEPERS_CINEMA].dat);
 rest(500);
 play_sample(datosIntro[SONIDO_INTERFERENCIA].dat,255,127,1000,FALSE);
 for (i=0;i<20;i++){
  vsync();
  blit (auxFoto,screen,0,0,70+((rand()%3)-1),25+((rand()%3)-1),auxFoto->w,auxFoto->h);
 }
 rest(1000);
 Fundir();
 set_palette(datosIntro[PALETA_JEEPERS_CINEMA].dat);
 auxFoto = datosIntro[FOTO_JEEPERS_PIEL].dat;
 blit (auxFoto,screen,0,0,30,0,auxFoto->w,auxFoto->h);
 rest(100);
 play_sample(datosIntro[SONIDO_JEEPERS_CAMIONETA].dat,255,127,1000,FALSE);
 rest(1000);
 AjustaPaleta(datosIntro[PALETA_JEEPERS_PIEL].dat,85);
// set_palette(datosIntro[PALETA_JEEPERS_PIEL].dat);
 rest(1000);
 Fundir();
 set_palette(black_palette);
 clear_bitmap (screen);
...
// play_midi(datosIntro[MIDI_INTRO].dat,FALSE);
 textout_centre(screen,datafile[FUENTE_GRANDE].dat,"Los coders estaban solos",160,75,255);
 Desfundir(datosIntro[PALETA_JEEPERS_CINEMA].dat);
 Fundir();
 clear_bitmap (screen);
 textout_centre(screen,datafile[FUENTE_GRANDE].dat,"En mitad de ninguna party",160,30,255);
 
 auxFoto = datosIntro[FOTO_JEEPERS_PARTY].dat;
 blit (auxFoto,screen,0,0,64,80,auxFoto->w,auxFoto->h);
 Desfundir(datosIntro[PALETA_JEEPERS_PARTY].dat);
 Fundir();
 clear_bitmap (screen);
 textout_centre(screen,datafile[FUENTE_GRANDE].dat,"Te esta vigilando",160,75,255);
 Desfundir(datosIntro[PALETA_JEEPERS_CINEMA].dat);
 Fundir();
 clear_bitmap (screen);

 for (i=0;i<128;i++)
  paletaGris[i].r = paletaGris[i].g = paletaGris[i].b = (i>>1);
 set_palette (paletaGris);
 textout_centre(screen,datafile[FUENTE_GRANDE].dat,"Cada 23 primaveras",160,55,127);
 fade_in_range (paletaGris,1,0,127);
// Desfundir(datosIntro[PALETA_JEEPERS_CINEMA].dat);
 rest (500);
 for (i=128;i<256;i++)
  paletaGris[i].r = paletaGris[i].g = paletaGris[i].b = ((i-128)>>1);
 set_palette (paletaGris);  
 textout_centre(screen,datafile[FUENTE_GRANDE].dat,"Y durante 23 dias",160,110,255);
 fade_in_range (paletaGris,1,128,255);
 rest (1000);
 Fundir();
 clear_bitmap (screen);
 textout_centre(screen,datafile[FUENTE_GRANDE].dat,"Le toca programar",160,75,255);
 Desfundir(datosIntro[PALETA_JEEPERS_CINEMA].dat);
 Fundir();
 clear_bitmap (screen);
 textout_centre(screen,datafile[FUENTE_GRANDE].dat,"Te esta esperando",160,75,255);
 Desfundir(datosIntro[PALETA_JEEPERS_CINEMA].dat);
 Fundir();

 clear_bitmap(screen);
 auxFoto = datosIntro[FOTO_CHICA].dat;
 set_palette(datosIntro[PALETA_JEEPERS_CINEMA].dat);
 blit (auxFoto,screen,0,0,64,0,auxFoto->w,auxFoto->h);
 play_sample(datosIntro[SONIDO_JEEPERS_GRITO].dat,255,127,1000,FALSE);
 AjustaPaleta(datosIntro[PALETA_CHICA].dat,70);
 Fundir();

 clear_bitmap (screen); 
 auxFoto = datosIntro[FOTO_JEEPERS_NUMERO1].dat;
 set_palette(datosIntro[PALETA_JEEPERS_NUMERO1].dat);
 play_sample(datosIntro[SONIDO_JEEPERS_CLEEPERS].dat,255,127,1000,FALSE);
 rest(1000);
 for (i=0,auxFlotante=10;i<50;i++,auxFlotante-=0.2){
  if (auxFlotante>1){
   clear_bitmap (pantallaVirtual);
   pivot_scaled_sprite(pantallaVirtual,auxFoto,160,100,58,50,itofix(0),ftofix(auxFlotante));
   vsync();
   blit (pantallaVirtual,screen,0,0,0,0,pantallaVirtual->w, pantallaVirtual->h);
  }
 }
 ...
 rest(500);
 EfectoTelevision(pantallaVirtual);
 ...
 while (!keypressed()){
  clear_bitmap (pantallaVirtual);
  for (i=0,auxFlotante=0;i<50;i++,auxFlotante+=8){
    clear_bitmap (pantallaVirtual);
    pivot_scaled_sprite(pantallaVirtual,auxFoto,160,100,95,100,ftofix(auxFlotante),itofix(1));
    vsync();
    blit (pantallaVirtual,screen,0,0,0,0,pantallaVirtual->w, pantallaVirtual->h);
  }  
 }
...
// readkey();
 ...
 //Efecto zoom con la chica y latidos de corazon
 auxFoto = datosIntro[FOTO_CHICA].dat;
 set_palette(datosIntro[PALETA_CHICA].dat);
 play_sample(datosIntro[SONIDO_LATIDO].dat,255,127,1000,TRUE);
 while (!keypressed()){
  for (i=0,auxFlotante=10;i<35;i++,auxFlotante-=0.2){
    clear_bitmap (pantallaVirtual);
    pivot_scaled_sprite(pantallaVirtual,auxFoto,160,100,95,100,itofix(0),ftofix(auxFlotante));
    vsync();
    blit (pantallaVirtual,screen,0,0,0,0,pantallaVirtual->w, pantallaVirtual->h);
  }
  for (i=0,auxFlotante=1;i<35;i++,auxFlotante+=0.2){
   clear_bitmap (pantallaVirtual);
   pivot_scaled_sprite(pantallaVirtual,auxFoto,160,100,95,100,itofix(0),ftofix(auxFlotante));
   vsync();
   blit (pantallaVirtual,screen,0,0,0,0,pantallaVirtual->w, pantallaVirtual->h);
 ...
 //readkey();
 clear_keybuf();
 stop_sample(datosIntro[SONIDO_LATIDO].dat);
 ...
 auxFoto = datosIntro[FOTO_JEEPERS_PIEL].dat;
 blit (auxFoto,screen,0,0,30,0,auxFoto->w,auxFoto->h);
 play_sample(datosIntro[SONIDO_RING].dat,255,127,1000,FALSE);
 rest(1900);
...
/* clear_bitmap(pantallaVirtual);
 clear_bitmap(screen);
 textout(screen,datafile[FUENTE_GRANDE].dat,"Ch 1",240,10,31);
 play_sample(datosIntro[SONIDO_TELEVISION].dat,255,127,1000,TRUE);
 while (!keypressed()){
  for (y=40;y<160;y+=2)
   for(x=20;x<300;x++)
    pantallaVirtual->line[y][x]= (rand()%8)+8;
  vsync();
  blit(pantallaVirtual,screen,20,40,20,40,280,120);
 }
 readkey();
 stop_sample(datosIntro[SONIDO_TELEVISION].dat);
 clear_bitmap(screen);*/
...
// set_palette(datosIntro[PALETA_JEEPERS_CINEMA].dat);
 ...
 if (pantallaVirtual!=NULL) //Eliminamos BITMAP auxiliares
  destroy_bitmap(pantallaVirtual);
 auxFoto = NULL; //Este lo igualo a NULL, porque no he reservado memoria

 unload_datafile(datosIntro);
// allegro_exit();
// exit(-1);
}
#endif
...
/**********************************************/
void AjustaPaleta(PALETTE paleta,short int bucle){
//Ajusta la paleta actual de video a una que queramos en tiempo real visual
 PALETTE paletaVideo;
 short int i,a;
 unsigned char r,g,b,r2,g2,b2;
 get_palette (paletaVideo);
 for (a=0;a<bucle;a++){
  for (i=0;i<=255;i++){
   r = paletaVideo[i].r; g = paletaVideo[i].g;  b = paletaVideo[i].b;
   r2 = paleta[i].r; g2 = paleta[i].g; b2 = paleta[i].b;
  
   if (r>r2) paletaVideo[i].r--;
   else
    if (r<r2) paletaVideo[i].r++;
   
   if (g>g2) paletaVideo[i].g--;
   else
    if (g<g2) paletaVideo[i].g++;
   
   if (b>b2) paletaVideo[i].b--;
   else
    if (b<b2) paletaVideo[i].b++;
  }
  vsync(); vsync();
  set_palette (paletaVideo);
 }
}

/*******************************************/
void EfectoTelevision(BITMAP *pantalla){
//Hace el efecto de television sin sintonizar con sonido y todo
 short int i,x,y;
 PALETTE paleta;

 //Efecto Television
 for (i=0;i<16;i++)
  paleta[i].r = paleta[i].g = paleta[i].b = (i<<2);
 for (i=16;i<32;i++){
  paleta[i].r = paleta[i].b = 0;
  paleta[i].g = ((i-16)<<2);
 }
 ...
 while (!keypressed()){
  for (y=40;y<160;y+=2)
   for(x=20;x<300;x++)
    pantalla->line[y][x]= (rand()%8)+8;
  vsync();
  blit(pantalla,screen,20,40,20,40,280,120);
 }
 clear_keybuf();
 stop_sample(datosIntro[SONIDO_TELEVISION].dat);
 Fundir();
